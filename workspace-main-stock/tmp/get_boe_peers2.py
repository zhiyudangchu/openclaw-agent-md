...
for ts_code, name in peer_companies:
    try:
        ...
        # 获取基本财务数据 - 使用 income 接口
        income = pro.income(ts_code=ts_code, fields='ts_code,ann_date,net_profit', start_date='20240101', end_date='20251231')
        ...
        # 获取每股净资产 - 使用 fina_per_share
        per_share = pro.fina_per_share(ts_code=ts_code, fields='ts_code,ann_date,bps,eps')
        if len(per_share) > 0:
            latest_bps = per_share.sort_values('ann_date', ascending=False).iloc[0]['bps']
            latest_eps = per_share.sort_values('ann_date', ascending=False).iloc[0]['eps']
        else:
            latest_bps = None
            latest_eps = None
        ...
        valuation_data.append({
            ...
            'ROE(%)': round(latest_roe, 2) if latest_roe else None,
            'EPS': round(latest_eps, 4) if latest_eps else None,
            'BPS': round(latest_bps, 2) if latest_bps else None,
        })
        
        print(f"  {ts_code}: 股价={latest_price:.2f}, 市值={market_cap:.2f}亿，PE={pe:.2f if pe else 'N/A'}, PB={pb:.2f if pb else 'N/A'}")
...
if valuation_data:
    ...
    # 计算行业平均和中位数
    pe_values = [x for x in df_valuation['PE'] if pd.notna(x)]
    pb_values = [x for x in df_valuation['PB'] if pd.notna(x)]
    ...
    if len(boe_row) > 0:
        boe_pe = boe_row.iloc[0]['PE']
        boe_pb = boe_row.iloc[0]['PB']
        
        if boe_pe and pe_values:
            pe_rank = sum(1 for x in pe_values if x < boe_pe) + 1
            print(f"\n京东方 A PE 排名：{pe_rank}/{len(pe_values)} (从小到大，数字越小估值越低)")
        
        if boe_pb and pb_values:
            pb_rank = sum(1 for x in pb_values if x < boe_pb) + 1
            print(f"京东方 A PB 排名：{pb_rank}/{len(pb_values)} (从小到大，数字越小估值越低)")
else:
    print("未能获取估值数据")
